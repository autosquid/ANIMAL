...
namespace CAPG{
    ...
    Euler Quaternion::toEuler(Euler::RotateOrder ro) const{
		Euler aEuler;
        aEuler.rotateOrder = ro;
        switch(ro) {
        case Euler::ZXY:
            ...
            break;
        case Euler::XYZ:
            aEuler.setX(-Math::ATan2(2 * m_fy * m_fz - 2 * m_fw * m_fx, 1 - 2 * m_fy * m_fy - 2 * m_fx * m_fx));
            aEuler.setY(Math::ASin(2 * m_fw * m_fy + 2 * m_fx * m_fz));
            aEuler.setZ(-Math::ATan2(2 * m_fx * m_fy - 2 * m_fw * m_fz, 1 - 2 * m_fy * m_fy - 2 * m_fz * m_fz));
            break;
        case Euler::YZX:
            aEuler.setY(-Math::ATan2(2 * m_fx * m_fz - 2 * m_fw * m_fy, 1 - 2 * m_fz * m_fz - 2 * m_fy * m_fy));
            aEuler.setZ(Math::ASin(2 * m_fw * m_fz + 2 * m_fx * m_fy));
            aEuler.setX(-Math::ATan2(2 * m_fy * m_fz - 2 * m_fw * m_fx, 1 - 2 * m_fz * m_fz - 2 * m_fx * m_fx));
            break;
        case Euler::XZY:
            aEuler.setX(Math::ATan2(2 * m_fy * m_fz + 2 * m_fw * m_fx, 1 - 2 * m_fz * m_fz - 2 * m_fx * m_fx));
            aEuler.setZ(Math::ASin(2 * m_fw * m_fz - 2 * m_fx * m_fy));
	...
	CAPG::Quaternion Quaternion::getComplexConjugate()const
	{
		return Quaternion(m_fw, -m_fx, -m_fy, -m_fz);
	}

	CAPG::Vector3 Quaternion::rotate( const Vector3& u )const
	{
		Vector3 v(m_fx,m_fy,m_fz);
		Vector3 t = u * m_fw + v.crossProduct(u);
		return v*u.dotProduct(v) + t * m_fw + v.crossProduct(t);
	}

	CAPG::Quaternion Quaternion::decomposeRotation( const Vector3& vB )const
	{
		//we need to compute v in A's coordinates
		Vector3 vA = rotate(vB);
		vA.normalise();

		//compute the rotation that aligns the vector v in the two coordinate frames (A and T)
		Vector3 rotAxis = vA.crossProduct(vB);
		rotAxis.normalise();
		Real val = vA.dotProduct(vB);
		val = std::max<Real>(val,-1);
		val = std::min<Real>(val,1);
		Real rotAngle = -acos(val);

		Quaternion TqA;
		TqA.fromAngleAxis(rotAngle, rotAxis*(-1));
		return TqA * (*this);
	}

	CAPG::Vector3 Quaternion::v() const
	{
